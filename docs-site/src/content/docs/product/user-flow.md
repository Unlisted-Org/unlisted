...
- **Its broken version failed, as it should.** With the tile deliberately showing the gross amount instead of what you receive, the run stopped at *paid now, exactly as the tile said*: expected 4,554,309, received 4,508,765, the difference being the 1% issuer fee (`web/e2e/BROKEN-VERSIONS.md`, *App stage*).
  - That run (13:08 UTC) had already redeemed on chain when its check failed ([`37QWahkv…QeGRwRCS`](https://explorer.solana.com/tx/37QWahkvrv6wz3oQ7XxLkLR7AQoeJLF7tTf2y7axREggq7GRX5rfwonbP8PPBYQ8U4aQxGCUwHftTDyuQeGRwRCS?cluster=devnet "slot 504002384")), leaving an ANTHROPIC claim of 89,490,895 units open on the canonical basket. It was later settled to its owner, permissionlessly: [`4KxbW87E…1TJPR8yS`](https://explorer.solana.com/tx/4KxbW87EnKAfcsTGnh7HpdCUvkYUoNgZ537Ju2ZHW9VKLSofKBZDkckkNWuY5KzXZxaHQZptXyAjMt6e1TJPR8yS?cluster=devnet "slot 504021122"). Both are now in the run's record.
  - **A failed run now settles its own open claims.** The test's cleanup settles any claim the run left open. A second broken-version run, at 14:02 UTC, failed at the same check and then settled its claim itself: [`4Co7QLeY…6FcvKzv5`](https://explorer.solana.com/tx/4Co7QLeYCp8wChoZp1i8uC6ypWTBB6AAGLcVSFXxoN2GqvZKD5BXFvLEkPpy5UJUJRwYwYLoDkgDRi9e6FcvKzv5?cluster=devnet).
