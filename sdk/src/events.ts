...
export type BasketEvent =
  ...
  | { name: "ClaimCreated"; owner: PublicKey; ticket: PublicKey; leg: number; units: bigint; reason: "Paused" | "Hook" | "Frozen" | "PendingSale" }
  | { name: "ClaimSettled"; owner: PublicKey; ticket: PublicKey; leg: number; units: bigint; amount: bigint; received: bigint }
...
// ClaimCreated.reason is ClaimReason (program@700004c).
const UNAVAILABLE = ["Paused", "Hook", "Frozen", "PendingSale"] as const;
...
const decoders: Record<string, (r: Reader) => BasketEvent> = {
  ...
  ClaimSettled: (r) => ({ name: "ClaimSettled", owner: r.pubkey(), ticket: r.pubkey(), leg: r.u8(), units: r.u64(), amount: r.u64(), received: r.u64() }),
...
export function encodeEvent(ev: BasketEvent): Uint8Array {
  ...
  switch (ev.name) {
    ...
    case "ClaimSettled": w.pubkey(ev.owner).pubkey(ev.ticket).u8(ev.leg).u64(ev.units).u64(ev.amount).u64(ev.received); break;
