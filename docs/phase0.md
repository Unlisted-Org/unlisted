...
1. **Permanent delegate can take from the vault without the vault signing.**
   ...
   - On mainnet the delegate is the 2-of-7 multisig above. The previous report scanned 963 of its transactions (March to September 2026) and found no use against holders in that window. **Update 2026-09-25:** an earlier window does contain use. On 2025-09-19 the delegate emptied 29 holder accounts to zero (see `docs/risks.md` §2).
...
| **Jupiter price v3 `usdPrice`** (`lite-api.jup.ag/price/v3`) | all 8 | "Last swapped price" (reported). Age by `blockId` against current slot, converted at the **measured 0.2657 s/slot** (corrected 2026-09-25; the first version assumed 0.4 s/slot and overstated ages by 1.5×): median / max. **FIGUREAI 50.3 / 55.5 min**; NEURALINK 6.6 / 18.7; SPACEX 2.3 / 14.3; KALSHI 2.1 / 5.5; ANDURIL 1.1 / 3.9; OPENAI 0.3 / 1.5; ANTHROPIC and POLYMARKET 0.2 / ~1.4. | Per UI token. `scaledUiConfig.usdPricePrescaled` = per raw unit. | Display, with its age shown |
...
- Last-trade prices are up to about 56 minutes stale, and the pools are thin enough ($99k–$888k Jupiter-reported liquidity) to move cheaply.
